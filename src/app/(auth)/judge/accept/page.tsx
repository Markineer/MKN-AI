...
export default function AcceptJudgeInvitationPage() {
  return (
    <Suspense fallback={
...
function AcceptJudgeContent() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/judges/accept?token=${token}`)
      .then(async (res) => {
        if (res.ok) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const res = await fetch("/api/judges/accept", {
        method: "POST",
      ...
      <div className="w-full max-w-md">
        ...
        {status === "expired" && (
          <div className="bento-card p-12 text-center animate-fade-in-up">
            ...
            <p className="text-sm text-gray-400 mb-4">يرجى التواصل مع منظم الفعالية لإرسال دعوة جديدة</p>
            <Link href="/login" className="btn-primary text-sm inline-flex items-center gap-2">
              الذهاب لتسجيل الدخول
              <ArrowLeft className="w-4 h-4" />
            </Link>
        ...
        {status === "success" && (
          <div className="bento-card p-12 text-center animate-scale-in">
            ...
            <p className="text-sm text-gray-400 mb-6">تم إنشاء حسابك كمحكّم. يمكنك الآن تسجيل الدخول بالبريد وكلمة المرور.</p>
            <Link href="/login" className="btn-primary text-sm inline-flex items-center gap-2">
              تسجيل الدخول الآن
              <ArrowLeft className="w-4 h-4" />
            </Link>
        ...
        {status === "valid" && inviteInfo && (
          <div className="bento-card overflow-hidden animate-fade-in-up">
            ...
            <div className="bg-gradient-to-l from-brand-600 via-brand-500 to-purple-600 p-6 text-white">
              <p className="text-purple-200 text-xs font-medium mb-2">دعوة تحكيم</p>
              <h2 className="text-lg font-bold mb-3">مرحباً بك كمحكّم</h2>
              <div className="space-y-1.5">
                <div className="flex items-center gap-2 text-sm">
                  <Calendar className="w-4 h-4 text-purple-200" />
                  <span className="text-purple-100">{inviteInfo.eventNameAr}</span>
                </div>
                {inviteInfo.trackNameAr && (
                  <div className="flex items-center gap-2 text-sm">
                    <MapPin className="w-4 h-4 text-purple-200" />
                    <span className="text-purple-100">{inviteInfo.trackNameAr}</span>
                  </div>
                )}
                <div className="flex items-center gap-2 text-sm">
                  <Shield className="w-4 h-4 text-purple-200" />
                  <span className="text-purple-100">محكّم</span>
                </div>
              </div>
            </div>
